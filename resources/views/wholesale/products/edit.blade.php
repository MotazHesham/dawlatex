            ...
            <div class="col-lg-8">
                ...
                <div class="card">
                    <ul class="nav nav-tabs nav-fill border-light language-bar">
                        ...
                        <li class="nav-item">
                            <a class="nav-link text-reset @if ($language->code == $lang) active @endif py-3" href="{{ route('wholesale_product_edit.admin', ['id'=>$product->id, 'lang'=> $language->code] ) }}">
                                ...
                                <div class="col-md-8">
                                    <select class="form-control aiz-selectpicker" name="publish_days" id="publish_days" required>
                                        ...
                                        @for ($i = 1 ; $i <= get_setting('product_publish_days') ; $i++)
                                            <option value="{{ $i }}" @if($product->publish_days == $i) selected @endif>{{ $i }}</option>
                                        @endfor
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="card-body">
                            <div class="h-300px overflow-auto c-scrollbar-light">
                                <select name="category_ids[0]" class="form-control" onchange="getSequanceCategory(this,1)" required>
                                    ...
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}" @if(in_array($category->id,$old_categories)) selected @endif>{{ $category->getTranslation('name') }}</option> 
                                    @endforeach
                                </select>
                                <div id="sequance-categories" class="mt-3">
                                    @foreach ($categories as $category) 
                                        @if(in_array($category->id,$old_categories))
                                            @include('backend.product.products.sequance_child_category', ['child_category' => $category,'old_categories' => $old_categories])
                                        @endif 
                                    @endforeach
                                </div>
                            </div>
